...
- **AI Assistant** — local (Ollama) or remote LLM, opt-in case context, **offline text-to-speech**,
  API keys encrypted.
...
> **Install:** download [`GhostAccess98-Setup-3.3.0.exe`](https://github.com/Obsidian-Circuit-LLC/ghost-access-98/releases/latest), verify the SHA-256, **More info → Run anyway** (unsigned).

## Status

**v3.3.0** — current release. A feature release on top of the v3.2.x base, with two full adversarial
red-team rounds:
...
Direct link to current release: [`GhostAccess98-Setup-3.3.0.exe`](https://github.com/Obsidian-Circuit-LLC/ghost-access-98/releases/download/v3.3.0/GhostAccess98-Setup-3.3.0.exe).

**Verify the download** before running it — compare its SHA-256 against the value in the release notes:

```powershell
Get-FileHash .\GhostAccess98-Setup-3.3.0.exe -Algorithm SHA256
# expected: 7f8ed773f995fbba6003a72aa8bf8d581be16f081ccd97cdb0b67280c727d214
```
...
| AI Assistant | Pluggable Ollama (local) / OpenAI-compatible providers, with an in-app **"Set up local AI"** wizard; case context is opt-in per message; API keys encrypted at rest. **Offline text-to-speech** reads replies aloud using on-device OS voices (cloud voices refused); **STFU** stops generation |

## Releases & changelog

The current build is **v3.3.0**. Each release page carries its own notes + SHA-256.
...
pnpm test         # vitest suite (189 tests as of v3.3.0)
...
- All network egress is initiated by an explicit user action and, for the newer modules, gated behind an explicit off-by-default toggle:
  ...
  - **AI text-to-speech** uses on-device OS voices only; cloud/"online" voices are refused so reply text never egresses.
