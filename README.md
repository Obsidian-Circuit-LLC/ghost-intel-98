...
> **Install:** download [`GhostIntel98-Setup-3.66.0.exe`](https://github.com/Obsidian-Circuit-LLC/ghost-intel-98/releases/latest), verify the SHA-256, **More info → Run anyway** (unsigned). *(Current build includes the Tor P2P chat — handshake **formally verified internally**: symbolic (ProVerif) + computational (CryptoVerif), internally adversarially reviewed; **not** independently audited and **not** FIPS-validated. See Status.)*
...
**v3.66.0** — **GeoINT Event Details dossier — Phase 3 (labeled offline-AI + honest Media).** The final dossier phase fills the MEDIA and INTEL SUMMARY tabs without inventing a fact. **INTEL SUMMARY** has three sections: **Key Entities** (places + named mentions pulled deterministically from the report's own text — no model needed, nothing typed as person/org the app can't back); **Reported casualty / verification phrases** — the *only* place casualty detail appears, and only as **verbatim quotes** labeled **"extracted · unverified"** (never a synthesized number or invented "verified" badge); and an **AI Summary** generated by **your own local Ollama model**, stamped **"AI · unverified"**, which summarizes the *unverified* report without judging its truth (no local model → the section says so and every other tab still works). The AI runs through a **new, isolated** path — Ollama-only (a remote/OpenAI-compatible endpoint is treated as "no local model", so unverified OSINT is never sent off-machine), SSRF-guarded, with **no web search, no RAG, no memory** — one loopback call, nothing else. The **MEDIA** tab is an honest **affordance** (it reports what media the source has + an Open-source button; it never inlines a remote `<img>`/`<video>`, which would beacon out). Built subagent-driven over 5 TDD tasks with a parallel adversarial whole-branch review weighted to AI-fabrication + egress, which caught a real cross-event summary misattribution (fixed) and tightened the extractors (Title-Case headlines no longer become one bogus entity; casualty matching is word-boundary-anchored). No new dependency. **3,778 automated tests** green (1 skipped); typecheck clean. *Everything from v3.65.0 carries forward.*
...
Direct link to the current release: [`GhostIntel98-Setup-3.66.0.exe`](https://github.com/Obsidian-Circuit-LLC/ghost-intel-98/releases/download/v3.66.0/GhostIntel98-Setup-3.66.0.exe)
...
```powershell
Get-FileHash .\GhostIntel98-Setup-3.66.0.exe -Algorithm SHA256
