...
> **Install:** download [`GhostAccess98-Setup-3.5.0.exe`](https://github.com/Obsidian-Circuit-LLC/ghost-access-98/releases/latest), verify the SHA-256, **More info → Run anyway** (unsigned).

## Status

**v3.5.0** — current release. Adds a **Markets** module (crypto/FX/indices/commodities, free keyless
sources, editable watchlist + custom feeds, egress-gated), a stronger **GeoINT** (satellite view, map
search, auto-refresh, button toggle, layout fix), **resizable Bookmarks** categories, a **CD-Player**
Jukebox, and **in-app playback of encrypted media** — all hardened by a pre-release red-team. The
v3.4.0 base added **offline voice conversation** to the AI Assistant: talk to it and
...
Direct link to current release: [`GhostAccess98-Setup-3.5.0.exe`](https://github.com/Obsidian-Circuit-LLC/ghost-access-98/releases/download/v3.5.0/GhostAccess98-Setup-3.5.0.exe).

**Verify the download** before running it — compare its SHA-256 against the value in the release notes:

```powershell
Get-FileHash .\GhostAccess98-Setup-3.5.0.exe -Algorithm SHA256
...
The current build is **v3.5.0**. Each release page carries its own notes + SHA-256.

- **v3.5.0** — **Markets module** (offline-first market overview, off by default: CoinGecko crypto, Frankfurter/ECB FX, Yahoo indices/equities/commodities, plus an editable watchlist and bring-your-own custom feeds); **GeoINT** gains a Street/Satellite toggle, place search, 5-min auto-refresh, an Enable/Disable button, and a fixed left-pane layout; **Bookmarks** categories are vertically resizable (and persist); the **Jukebox** is restyled to the Win98 CD Player; and **encrypted media now plays in-app** (decrypt-and-play, plaintext never on disk). A full pre-release **red-team** fixed every finding — notably a DNS-aware SSRF guard and fetch timeout/size caps on all outbound calls. 218 tests.
