 ...
 */
import { ensureEmbedRuntime, embedEndpoint } from './embed-runtime';
...
/** Default embedder: ensure the dedicated embed runtime is up, then embed each text via /api/embeddings. */
const defaultEmbed: Embedder = async (texts) => {
  if (texts.length === 0) return [];
  await ensureEmbedRuntime();
  const out: number[][] = [];
  for (const prompt of texts) {
    const res = await fetch(`${embedEndpoint()}/api/embeddings`, {
      method: 'POST',
