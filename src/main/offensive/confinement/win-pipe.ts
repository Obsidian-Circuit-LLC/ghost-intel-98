 ...
 *
 * Control messages (REQUEST/RESPONSE) are JSON; child stdout/stderr/exit are streamed as frames so
 * ConfinedIO works exactly as it does for the Linux netns jail. The codec is pure and unit-tested on
 * Linux; the actual pipe transport (\\.\pipe\dcs98-confine, Windows-only) is injected in win-wfp.ts.
 */
...
/** Control requests the app sends to the service. `filters` is the buildWfpFilterSpec output. */
