...
describe('searchlight channels', () => {
  it('exposes the expected channel set, all namespaced under searchlight:', () => {
    ...
    expect(g).toBeTruthy();
    const expected = ['catalog', 'startSweep', 'cancelSweep', 'importSites', 'listCases', 'saveCase', 'loadCase', 'deleteCase', 'exportCase', 'importCase', 'onSweepResult', 'onSweepDone', 'favicon', 'addCustomSite', 'exportSites', 'exportPdf', 'saveReport', 'torStatus', 'connectTor', 'revealSiteDbDir', 'labelResult', 'learningStatus', 'trainModel', 'setMlEnabled'];
