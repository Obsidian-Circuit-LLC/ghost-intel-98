...
describe('win-pipe frame codec', () => {
  it('round-trips a control request across arbitrary chunk boundaries', () => {
    ...
    for (const b of wire) out.push(...dec.push(Buffer.from([b]))); // one byte at a time
  ...
  it('preserves binary stdout payloads exactly', () => {
    const bin = Buffer.from([0x00, 0xff, 0x10, 0x7f, 0x80]);
    const dec = new FrameDecoder();
    const out = dec.push(encodeFrame(FRAME.STDOUT, bin));
    expect(out[0].body.equals(bin)).toBe(true);
  });
