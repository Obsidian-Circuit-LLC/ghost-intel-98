...
- **Commits:** persona `onna-bugeisha-dev-team <dev@onna-bugeisha.org>`. **Never** emit `Co-Authored-By:` / `Signed-off-by:` / `Claude-Session:` trailers in author, committer, or message body.
...
- Charter: X quarantine intact (no embed); commits carry the `onna-bugeisha-dev-team` persona with no AI trailers.
