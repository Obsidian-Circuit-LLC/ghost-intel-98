...
**Status:** design complete; **fixtures captured + verified 2026-07-05 → engine set finalized, ready to build.** Real fixtures were captured on GhostExodus's Tor box (`test/fixtures/search/`). Verification outcome (verify-or-drop): **SearXNG onion JSON works** (11 clean `{url,title,content,engine}` results, aggregates Bing/Google/etc. server-side, onion-to-onion IP-hidden); **DuckDuckGo onion** control confirms the capture is faithful. **Google / Bing / Yandex are DEAD** — all returned scraping blocks even from a residential IP (Google no-JS "enable JavaScript"; Bing `captchaSuccessPostMessage` verification; Yandex "Verification" captcha). **Yahoo** returned results but redirect-wrapped + widget-mixed (fragile) — dropped. **Shipped engine set = DuckDuckGo + SearXNG, both `tor`/onion.** The `clearnet` egress tier ships **empty** (no viable clearnet engine) — the `egress` field + gating stay in the design as cheap future scaffolding, but no clearnet parser is built. SearXNG default instance = `searxokthnxmo7ndis35jpts2tawcwvbovuy47qtavwo7oq4jgcm5gqd.onion` (operator's verified pick; editable, fail-closed, `.onion`-enforced). This supersedes §5's candidate list.
**Date:** 2026-07-05
