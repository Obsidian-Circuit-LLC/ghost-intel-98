...
- **Live News (video)** — a small HLS video panel (top-right) playing a streaming news feed, **muted autoplay**, via the `hls.js` the app already uses (EyeSpy Viewer). Seeded with the Bloomberg US stream (`https://www.bloomberg.com/media-manifest/streams/us.m3u8`); the user can **save their own `.m3u8` feeds into a playlist and pick from a dropdown** (persisted in `settings.geoint.newsStreams: {label,url}[]` with a selected index). Gated behind `networkEnabled` (live video is real egress); URLs validated (public-HTTP, `.m3u8`). Note: shipping a default third-party stream is a light ToS consideration — the entry is user-removable and the panel is fundamentally "play the stream you choose," like EyeSpy cameras.
