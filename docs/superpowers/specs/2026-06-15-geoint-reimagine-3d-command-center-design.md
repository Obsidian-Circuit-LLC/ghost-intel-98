...
- **Pin layers:** USGS earthquakes (free, DONE), GDACS (free, DONE). **GDELT** — official GEO 2.0 is DOWN (HTTP 404, 2026-06-15); operator chose the **GDELT DOC API** (free, no-key, but article-level → **country-centroid** news-signal, chatter-labeled; geo quality limited) + gdeltcloud.com optional user-key. **UCDP** — now **token-gated** (free `x-ucdp-access-token`, CC BY 4.0 still) → **user-key conflict layer** (operator: keep). NASA FIRMS (user MAP_KEY), war-tracker.com (optional, free social-OSINT chatter + AIS maritime, labeled unverified). **ACLED DROPPED** (UCDP covers conflict; no EULA/OAuth). Cloudflare Radar **excluded**.
...
- **Breaking News Feed / Situation Feed** — categorized headlines from the user's existing feed sources + GDELT, color-tagged by the existing category classifier (conflict/cyber/protest/disaster/crime/politics). Clicking an item flies the map to its location (reuses geocode + flyTo).
  - **Custom Feed enhancement (GhostExodus 2026-06-15):** the feed source system already supports RSS/Atom/GeoJSON/KML/GPX/XML with polling + normalization. ADD **JSON Feed** (jsonfeed.org) as a source type (`feeds.ts` parser + `detectType` + Sources dropdown) and an optional **`image`** field on the normalized entry/`GeoItem`. This lets users paste feeds from RSS.app / RSSHub / Feedly / Inoreader / Google News / Mastodon / Bluesky / Telegram bridges / GitHub releases / CVE feeds — and **X/Twitter via a bridge** (X has NO native RSS; Nitter is fragile — do NOT build on it; users generate an RSS/JSON URL via RSS.app/RSSHub and paste it). "Webhook URL" = **a pollable URL** the user pastes (e.g. a bridge endpoint), NOT a local inbound listener — this app opens no inbound port (OpSec). The Situation Feed panel renders the normalized entries (title/source/timestamp/link/summary/image/severity).
