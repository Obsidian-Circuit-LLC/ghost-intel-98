...
- **In-app header — banner image (delivered):** a branded header strip at the top of `InvoicesModule` renders the bundled banner **`src/renderer/assets/ghost-ledger-banner.png`** — the "GHOST LEDGER 98 · PROFESSIONAL BILLING & INVOICING" artwork extracted from the operator's mockup (1669×247, ~6.76:1, already placed in the repo). Imported like the existing `logo.png` (`import bannerUrl from '../../assets/ghost-ledger-banner.png'`) and rendered **full-width** — `.ga98-ledger-banner { display:block; width:100%; height:auto; }` — so it spans the module top and scales with its width (≈133px tall at the 900px default, matching the mockup), filling the empty header space GhostExodus flagged. No text-wordmark fallback needed (the asset is in hand).
